...
Page({
    ...
    wx.request({
      ...
      success: function(res){
        if(res.data.result)
        {
          ...
          model.GroupPrice = model.OriginalPrice - price;
          jianprice = price
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      path: '/newpage/Coupcour/Coupcour?id=' + that.data.kid + '&yu=0&csid=0&price=0&copenid=' + wx.getStorageSync('openid')
    }
  },
   Xiadan:function(e){
     var that=this;
     ...
     pingdian = "false";
     wx.navigateTo({
       url: '../CoupOuder/CoupOuder?kid=' + kid + '&title=' + that.data.model.Title + "&img=" + that.data.model.PicturePath + "&op=" + that.data.oprice + "&jp=" + that.data.jianprice +"&csid="+that.data.csid
     });
     }
 },
   zixun:function(){
