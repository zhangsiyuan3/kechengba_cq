// newpage/envelope/envelope.js
var common = require('../../Common.js');

Page({

  /**
   * 页面的初始数据
   */
  data: {
    imgbg: wx.getStorageSync('avatarUrl'),
    bgRed: common.config.StaticPath +"RedBg_03.png",
    backimg: "",
    model:false,
    modelTwo: true,
    setmine:true,
    xian: common.config.StaticPath+"xian-left_03.png",
    mm:{},
    sheng:0,
    headlist:[]
  },
  /**
   * 生命周期函数--监听页面加载
   */
  onLoad: function (options) {
    var that=this;
    var esid=options.esid;
    var copenid =options.copenid;
    var numbertime = options.numbertime;

    if (numbertime == undefined) {
      numbertime = "0";
    }

    wx.setNavigationBarTitle({ title: common.data.TitleName });
    var openid = "";
    if (wx.getStorageSync('openid') == null || wx.getStorageSync('openid') == "") {
      wx.login({
        success: function (res) {
          if (res.code) {
            //获取code
            wx.setStorageSync('Code', res.code);
            // wx.getUserInfo({
            //   success: function (res) {
            var s = {}//JSON.parse(res.rawData);
            var nickName = wx.getStorageSync('nickName')//s.nickName;//昵称
            var avatarUrl = wx.getStorageSync('avatarUrl')//s.avatarUrl;//头像
            // wx.setStorageSync("nickName", s.nickName);//昵称
            // wx.setStorageSync("avatarUrl", s.avatarUrl);//头像
            wx.request({
              url: common.config.GetOrSetOpenid,
              data: {
                code: wx.getStorageSync('Code'),
                name: nickName,
                img: avatarUrl,
                mchid: common.data.MchId
              },
              header: {
                'content-type': 'application/json'
              },
              method: 'POST',
              success: function (res) {
                console.log("WHITEENVOLOPE_GetOrSetOpenid",res)
                if (res.data.res) {
                  wx.setStorageSync('openid', res.data.data.openId);
                  openid = res.data.data.openId;
                  wx.getLocation({
                    type: 'wgs84',
                    success: function (res) {
                      //2、根据坐标获取当前位置名称，显示在顶部:腾讯地图逆地址解析
                      common.demo.reverseGeocoder({
                        location: {
                          latitude: res.latitude,
                          longitude: res.longitude
                        },
                        success: function (addressRes) {
                          var sheng = addressRes.result.address_component.province;
                          var shi = addressRes.result.address_component.city;
                          var address = addressRes.result.address;
                          wx.setStorageSync("province", sheng);//省缓存
                          wx.setStorageSync("city", shi);//市缓存
                          wx.setStorageSync("address", address);//地址缓存
                        }
                      })
                    }
                  })
                  that.fangfa(openid, esid, copenid, numbertime);
                }
              }
            })
              //}
            //})
          }
        },
        fail: function (res) { //用户无授权时
          that.setData({
            getUserInfoFail: true
          })
        }
      });
    }
    else {
      openid = wx.getStorageSync('openid');
      if(openid==copenid)
      {
        wx.redirectTo({
          url: '../Twoenvelope/Twoenvelope?esid=' + esid + "&chai=1"
        });
      }
      else{
        that.fangfa(openid, esid, copenid, numbertime);
      }
    }
  },
  /**
   * 生命周期函数--监听页面初次渲染完成
   */
  onReady: function () {
  
  },

  /**
   * 生命周期函数--监听页面显示
   */
  onShow: function () {
  
  },

  /**
   * 生命周期函数--监听页面隐藏
   */
  onHide: function () {
  
  },

  /**
   * 生命周期函数--监听页面卸载
   */
  onUnload: function () {
  
  },

  /**
   * 页面相关事件处理函数--监听用户下拉动作
   */
  onPullDownRefresh: function () {
  
  },

  /**
   * 页面上拉触底事件的处理函数
   */
  onReachBottom: function () {
  
  },
  fangfa: function (openid, esid, copenid, numbertime) {
    wx.request({
      url: common.config.InsertEnvelopeSplitsUsers,
      data: {
        openid: openid,
        esid: esid,
        copenid: copenid,
        mchid: common.data.MchId,
        province: wx.getStorageSync('province'),
        city: wx.getStorageSync('city'),
        address: wx.getStorageSync('address'),
        number: numbertime
      },
      method: 'POST',
      header: {
        'content-type': 'application/json'
      },
      success: function (res) {
        var types = res.data.types;
        if (types == "1" && res.data.status == "chaiok") {
          wx.redirectTo({
            url: '../Twoenvelope/Twoenvelope?esid=' + esid + "&chai=1"
          });
        }
        if (types == "2" && res.data.status == "chaiok") {
          wx.redirectTo({
            url: '../Aready/Aready?esid=' + esid
          });
        }
        else if (types == "1" && res.data.status == "chaiguo") {
          wx.redirectTo({
            url: '../Twoenvelope/Twoenvelope?esid=' + esid + "&chai=0"
          });
        }
        else if (types == "2" && res.data.status == "chaiwan") {
          wx.redirectTo({
            url: '../Aready/Aready?esid=' + esid
          });
        }
        else if (types == "2" && res.data.status == "chenggong") {
          wx.redirectTo({
            url: '../Aready/Aready?esid=' + esid
          });
        }
        else if (types == "3" && res.data.status == "guoqi") {
          wx.redirectTo({
            url: '../Twoenvelope/Twoenvelope?esid=' + esid + "&chai=0"
          });
        }
      }
    });
  }
})