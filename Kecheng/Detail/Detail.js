...
Page({
  ...
  toPart(){
    let { kid } = this.data
    if(!kid) return
    wx.navigateTo({ url : `/Kecheng/part/part?kid=${kid}` })
  },
  getPartNum(courseId) { //获取参与人数
    wx.request({
        ...
        if (res.data.result) {
          ...
          if (model.Status != "2") {
            ...
            for (var i = 0; i < group.length; i++) {
              ...
              var zhu = false;//开团者
              if (group[i].ColonelOpenId == openid) { zhu = true; }
              ...
              let endTime = +group[i].EndTime.replace(/\D/g,'');
              if(nowTime < endTime) glist.push(b);
            ...
            WxParse.wxParse('article', 'html', model.Description, that, 20);
            that.setData({
              movies: list,
              kid: id,
              group: glist,
