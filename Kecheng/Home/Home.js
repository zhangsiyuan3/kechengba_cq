...
Page({
  ...
  data: {
    ...
    html: {
      a: common.config.StaticPath + "tuijia_02.jpg",
      b: common.config.StaticPath + "home_sou.jpg",
      c: common.config.StaticPath + "remai_03.png"
    },
  ...
  onShow: function() {
    if (pin != true) {
      ...
      // console.log(this.data.tabCourList)
      this.onLoad();
    }
    wx.setNavigationBarTitle({
      title: common.data.TitleName
  ...
  onPullDownRefresh: function() {
    ...
    if (homePage.data.currentTab == 0) {
      ...
      courTC = [];
      // var cIds = this.data.cIds; //所包含课程的Id
            ...
            success: function (res) {
              ...
              if (res.data.result) {
                ...
                if (kelist != null && kelist.length > 0) {
                  ...
                  //tabCourList = tabCourList.concat(cList)
                  homePage.setData({
                    tabCourList: cList
                  })
                  countDown(homePage);
      ...
      success: function(res) { //请求成功
        ...
        for (var i = 0; i < tools.length; i++) {
          var tool = {
            ...
            courIds: tools[i].CourseListString,
            Id: tools[i].Id
          };
        ...
        if (modules !== null && modules.length > 0) {
          for (let i = 0; i < modules.length; i++){
            if (modules[i].CourType != 9) {
              //console.log("分销员--------------")
              wx.setStorageSync('fenxiao', false);
            } else {
              //console.log("分销员+++++++++++++++")
              wx.setStorageSync('fenxiao', true);
              break;
            }
          }
    ...
    wx.request({
      ...
      success: function(res) { //请求成功
        ...
        homePage.setData({
          navContent: navContent,
          navWidth: homePage.data.navWidth,
          currentTab: homePage.data.currentTab
        });
      },
      complete: function() {
        homePage.getTabPage();
  ...
  navbarTap: function(e) {
    console.log("导航栏选中",e)
    console.log("TOOLS",this.data.tools)
    ...
    if (cIds != "07" && cIds != "08") {
      ...
      if (homePage.data.currentTab == 0) {
        ...
        //this.getIndexDatas(openid, atId)
  ...
  keke: function(e) {
    ...
    if (types == "7" || types ==  15) {
      //let re_types = types == 15 ? types = 8 : types;
      if (types == "7"){
        types = "0" + types
      }else{
        types = +types
