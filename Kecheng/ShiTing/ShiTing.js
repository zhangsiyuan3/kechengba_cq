...
Page({
  ...
  onShareAppMessage: function () {
    ...
    return {
      ...
      path: '/Kecheng/ShiTing/ShiTing?id=' + that.data.kid + '&copenid=' + wx.getStorageSync('openid') + '&numbertime=' + nn,
      success: function (res) {
   ...
   Xiadan:function(e){
     ...
     if(pingdian=="false" || that.data.oid!="0")
     {
       return;
     }else{
     pingdian = "false";
     wx.navigateTo({
       url: '../ShiTingOuder/ShiTingOuder?kid=' + kid + '&title=' + that.data.model.Title + "&img=" + that.data.model.PicturePath + "&op=" + that.data.oprice + "&jp=" + that.data.gprice
     });
     }
 },
   detial:function(){
