...
let quan_id = "",quan_name = "",quan_money = 0
...
Page({
  data: {
    ...
    clock: '',
    html: { a: common.config.StaticPath + "tuijia_02.jpg", b: common.config.StaticPath + "home_sou.jpg", c: common.config.StaticPath + "remai_03.png" },
  ...
  onLoad: function (options) {
    ...
    if (types) {
      console.log("T_Y_P_E_S",types)
      types = +types
      switch (types) {
        case 1://团购课程
          wx.setNavigationBarTitle({ title: '团购课程' });
          break;
        case 4://砍价课程
          wx.setNavigationBarTitle({ title: '砍价' });
          break;
        case 5://一元课程
          wx.setNavigationBarTitle({ title: '一元好课' });
          break;
        case 6://答题课程
          wx.setNavigationBarTitle({ title: '答题赢课' });
          break;
        case 10://优惠课程
          wx.setNavigationBarTitle({ title: '优惠课程' });
          break;
        case 11://试听课程
          wx.setNavigationBarTitle({ title: '试听课' });
          break;
        default:
          wx.setNavigationBarTitle({ title: common.data.TitleName });
          break;
  ...
  fanye: function (types,val) {
    ...
    if (ppage != -1) {
      ...
      wx.request({
        ...
        method: 'POST',
        data: { 
          types: types, status: 2, 
          page: ppage, 
          openid: openid, 
          adminsmchid: common.data.MchId,
          seaK:val
        },
        header: {
          'content-type': 'application/json'
        ...
        success: function (res) {
          ...
          if (res.data.result) {
            ...
            for (var i = 0; i < kelist.length; i++) {
              console.log('kelist', kelist)
              if (kelist[i].Title.length > 20){
                kelist[i].Title = kelist[i].Title.substr(0,18)+ "..."
              }
  ...
  Ping: function (e) {
    ...
    if (types == "4") //砍价
    {
      if (scount != "0") {
        if (gid == "0") {
          ...
          wx.navigateTo({
            url: '../Twobargaining/Twobargaining?gid=' + gid + "&yu=0&cid=" + Id
          })
        }
      }
    }
    ...
    if (types == '10')//优惠
    {
      wx.navigateTo({
        ...
        `&quan_id=${quan_id}&quan_name=${quan_name}&quan_money=${quan_money}`
      });
    }
    if (types == "11")//试听
    {
