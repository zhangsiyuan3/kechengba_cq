      ...
      wx.request({
        ...
        success: function (res) {
          if (res.data.result) {
            ...
            if (model.Course.OriginalPrice){
              model.Course.OriginalPrice = model.Course.OriginalPrice.toFixed(2)
            }
            //
            ...
            if (otype == "1" ||  otype =="4") {
              ...
              status = model.Type;
              model.StartTime = util.formatTime(new Date(+model.CreateOn.replace(/[^0-9]/ig, "")))
              model.EndTime = util.formatTime(new Date(+model.CreateOn.replace(/[^0-9]/ig, "")))
            if (model.Course.GroupPrice){
              model.Course.GroupPrice = model.Course.GroupPrice.toFixed(2)
            }
              //model.CreateOn = common.timeStamp2String(model.CreateOn);
              // model.StartTime = model.CreateOn;
              // model.EndTime = model.CreateOn;
            }
            console.log("MODEL__",res.data.model)
            //更新数据
            that.setData({
              title: title,
              img: img,
