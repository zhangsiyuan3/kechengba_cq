...
Page({
  data: {
    clock: '',
    jiao: common.config.StaticPath + "jiao.jpg",
    userInfo: {},
    show: true,
    kid:"",
    gid:"",
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success: function (res) {
        ...
        //   return
        // }
        that.GetGropuBookingOfId(gid,openid)
      }
    });
  },
  GetGropuBookingOfId(gid){
    let that = this
        ...
        if (res.data.result) {
          ...
          for (var i = 0; i < parseInt(ucount) - 1; i++) {
            ...
            if (i + 1 < glist.length) {
              ...
              im = {
                im: 'https://altest.1-zhao.com/YHJImage/pintuan.png'//common.config.StaticPath + "yuan.jpg" 
              };
            }
            list.push(im);
          }
          for (var j = 0; j < glist.length; j++) {
            if (glist[j].UsersOpenId == openid) { a = true }
          }
          var d1 = new Date();
